...
class StoneSerializerBase(StoneEncoderInterface):
    ...
    def __init__(self, caller_permissions, alias_validators=None):
        ...
        Args:
            ...
            alias_validators (``typing.Mapping``, optional): A mapping of
                ...
                Defaults to ``None``.
        """
        self.caller_permissions = caller_permissions
...
class StoneToPythonPrimitiveSerializer(StoneSerializerBase):
    ...
    def __init__(self, caller_permissions, alias_validators=None, for_msgpack=False, old_style=False):
        ...
        Args:
            ...
            old_style (bool, optional): See the like-named property.
                Defaults to ``False``.
        """
        super(StoneToPythonPrimitiveSerializer, self).__init__(caller_permissions, alias_validators=alias_validators)
    ...
    def encode_union(self, validator, value):
        ...
        if not validator.definition._is_tag_present(value._tag, self.caller_permissions):
            raise bv.ValidationError('caller does not have access to this tag')
...
class StoneToJsonSerializer(StoneToPythonPrimitiveSerializer):
    def __init__(self, caller_permissions, alias_validators, for_msgpack, old_style):
        super(StoneToJsonSerializer, self).__init__(caller_permissions, alias_validators,
                                                    for_msgpack, old_style)
...
def json_encode(data_type, obj, caller_permissions, alias_validators=None, old_style=False):
...
def json_compat_obj_encode(data_type, obj, caller_permissions, alias_validators=None,
...
class PythonPrimitiveToStoneDecoder(object):
    ...
    def decode_struct(self, data_type, obj):
        ...
        for extra_permission in self.caller_permissions.permissions:
            permissioned_fields_map = '_all_{}_fields_'.format(extra_permission)
            all_fields = all_fields + getattr(data_type.definition, permissioned_fields_map, [])

        if self.strict:
            all_field_names = data_type.definition._all_field_names_
            for extra_permission in self.caller_permissions.permissions:
                permissioned_field_names_map = '_all_{}_field_names_'.format(extra_permission)
                all_field_names = all_field_names.union(getattr(data_type.definition,
                                                                permissioned_field_names_map, {}))
        ...
        if self.caller_permissions.permissions:
            data_type.validate_with_permissions_fields_only(ins, self.caller_permissions)
...
def json_decode(data_type, serialized_obj, caller_permissions,
    ...
    try:
        ...
        return json_compat_obj_decode(
            data_type, deserialized_obj, caller_permissions,
            alias_validators, strict, old_style)


def json_compat_obj_decode(data_type, obj, caller_permissions,
