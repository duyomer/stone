...
class PythonTypesBackend(CodeBackend):
    ...
    def _generate_struct_class_reflection_attributes(self, ns, data_type):
        ...
        # generate `_all_field_names_` and `_all_fields_` for every omission group (and public)
        # union `omission_groups` with None in the case when object has no public fields as
        # we still need to generate public attributes (`_field_names_` etc)
        child_omission_groups = data_type.get_omission_groups() | {None}
        parent_omission_groups = data_type.parent_type.get_omission_groups() if \
            data_type.parent_type else set([])

        for omission_group_name in child_omission_groups | parent_omission_groups:
            is_public = omission_group_name is None
            map_name_prefix = '' if is_public else '_{}'.format(omission_group_name)
            group_in_parent = data_type.parent_type and (is_public or omission_group_name
                                                         in parent_omission_groups)

            def field_in_group(group):
                return (group and group.group_name) == omission_group_name

            # generate `_all_field_names_`
            names_map_name = '{}_field_names_'.format(map_name_prefix)
            nonnull_names_map_name = '{}_nonnull_field_names_'.format(map_name_prefix)
            all_names_map_name = '_all{}_field_names_'.format(map_name_prefix)
            all_nonnull_names_map_name = '_all{}_nonnull_field_names_'.format(map_name_prefix)
            if data_type.is_member_of_enumerated_subtypes_tree():
                if is_public or omission_group_name in child_omission_groups:
                    self.generate_multiline_list(
                        [
                            "'%s'" % field.name
                            for field in data_type.fields
                            if field_in_group(field.omission_group)
                        ],
                    ...
                    if not is_public:
                        self.generate_multiline_list(
                            [
                                "'%s'" % field.name
                                for field in data_type.fields
                                if field_in_group(field.omission_group) and not
                                is_nullable_type(field.data_type)
                            ],
                            before='{}.{} = set('.format(class_name, nonnull_names_map_name),
                            after=')',
                            delim=('[', ']'),
                            compact=False)
                if group_in_parent:
                    ...
                    if not is_public:
                        self.emit('{0}.{3} = {1}.{3}.union({0}.{2})'
                                  .format(class_name, parent_type_class_name,
                                          nonnull_names_map_name, all_nonnull_names_map_name))
                else:
                    self.emit('{0}.{2} = {0}.{1}'.format(class_name, names_map_name,
                                                         all_names_map_name))
                    if not is_public:
                        self.emit('{0}.{2} = {0}.{1}'.format(class_name, nonnull_names_map_name,
                                                             all_nonnull_names_map_name))
            else:
                if group_in_parent:
                    before = '{0}.{1} = {2}.{1}.union(set('.format(class_name, all_names_map_name,
                                                                   parent_type_class_name)
                    after = '))'
                    before_nonnull = '{0}.{1} = {2}.{1}.union(set('.format(
                        class_name, all_nonnull_names_map_name, parent_type_class_name)
                    after_nonnull = '))'
                else:
                    before = '{}.{} = set('.format(class_name, all_names_map_name)
                    after = ')'
                    before_nonnull = '{}.{} = set('.format(class_name, all_nonnull_names_map_name)
                    after_nonnull = ')'
                items = [
                    "'%s'" % field.name
                    for field in data_type.fields
            ...
            if data_type.is_member_of_enumerated_subtypes_tree():
                items = []
                for field in data_type.fields:
                    if not field_in_group(field.omission_group):
                        continue
    ...
    def _generate_struct_class_properties(self, ns, data_type):
        ...
        for field in data_type.fields:
            ...
            with self.indent():
                ...
                self.emit('else:')
                with self.indent():
                    if dt_nullable or field.omission_group:
    ...
    def _generate_union_class_reflection_attributes(self, ns, data_type):
        ...
        # generate _all_fields_ for every omission group (and public)
        child_omission_groups = data_type.get_omission_groups()
        parent_omission_groups = data_type.parent_type.get_omission_groups() if \
            data_type.parent_type else set([])

        all_omision_groups = child_omission_groups | parent_omission_groups
        if len(all_omision_groups) != 0:
            self.emit('{}._permissioned_tagmaps = {}'.format(class_name, all_omision_groups))
        for omission_group_name in all_omision_groups | {None}:
            is_public = omission_group_name is None
            tagmap_name = '_tagmap' if is_public else '_{}_tagmap'.format(omission_group_name)
            group_in_parent = data_type.parent_type and (is_public or omission_group_name
                                                         in parent_omission_groups)
